 ...
 * Exporte la base locale sans les vues ET sans les clés étrangères (compatibles InfinityFree)
 */
...
try {
    ...
    echo "<h2>📤 Export de la base de données ASOD ACADEMIE (SANS clés étrangères)</h2>";
    ...
    $sqlFile = 'asod_academie_export_no_fk_' . date('Y-m-d_H-i-s') . '.sql';
    ...
    fwrite($file, "-- Export de la base de données ASOD ACADEMIE (SANS clés étrangères)\n");
    ...
    foreach ($tables as $table) {
        ...
        // Structure de la table (sans clés étrangères)
        $createTable = $pdo->query("SHOW CREATE TABLE `$table`")->fetch();
        $createTableSql = $createTable['Create Table'];
        
        // Supprimer les contraintes de clés étrangères
        $createTableSql = preg_replace('/,\s*CONSTRAINT\s+`[^`]+`\s+FOREIGN KEY\s+\([^)]+\)\s+REFERENCES\s+`[^`]+`\s+\([^)]+\)[^,]*/', '', $createTableSql);
        $createTableSql = preg_replace('/,\s*FOREIGN KEY\s+\([^)]+\)\s+REFERENCES\s+`[^`]+`\s+\([^)]+\)[^,]*/', '', $createTableSql);
    ...
    echo "<p style='color: orange;'>⚠️ <strong>Note :</strong> Ce fichier ne contient pas de clés étrangères pour éviter les erreurs d'import sur InfinityFree.</p>";
